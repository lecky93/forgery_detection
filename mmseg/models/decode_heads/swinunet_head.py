...
class SwinTransformerUnetHead(BaseDecodeHead):
    def __init__(self, img_size=224, patch_size=4,
                 embed_dim=96, depths_decoder=[1, 2, 2, 2], num_heads=[3, 6, 12, 24],
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, for_se_mask = False,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, final_upsample="expand_first", **kwargs):
        super(SwinTransformerUnetHead, self).__init__(input_transform='multiple_select', **kwargs)
        self.for_se_mask = for_se_mask
        self.embed_dim = embed_dim
        self.img_size = img_size
    ...
    def forward_up_features(self, inputs):
        ...
        for i in range(len(inputs)):
            inputs[i] = inputs[i].view(B, H*W, C)
