...
class AttentionHead(BaseDecodeHead):
    def __init__(self,
        ...
        for i in range(num_layers):
            concat_linear = nn.Linear(in_channels + self.in_channels[-1-i], in_channels) if i > 0 else nn.Identity()

            if i < num_layers - 1:
                upsample = PatchExpand(in_channels)
            else:
                upsample = None
    ...
    def up_x4(self, x):
        ...
        x = resize(x, size=self.img_size, mode='bilinear', align_corners=self.align_corners)
    ...
    def forward(self, inputs):
        ...
        for i, stage in enumerate(self.stages):
            if i > 0:
                x = torch.cat([x, inputs[-1 - i]], -1)
                x = self.concat_back_dim[i](x)
            x = stage(x, hw_shape[-1-i])
