...
class ConstrainedConvNeXt(BaseModule):
    """ConvNeXt.
    ...
    def __init__(self,
        ...
        self.constrain_conv = BayarConv2d(in_channels=3, out_channels=3, padding=2)
    ...
    def train(self, mode=True):
        super(ConstrainedConvNeXt, self).train(mode)
        self._freeze_stages()


if __name__ == '__main__':
    x = torch.rand(4, 3, 512, 512).cuda()
    model = ConstrainedConvNeXt(arch='base', out_indices=3, drop_path_rate=0.5, gap_before_final_norm=False).cuda()
    # from tensorboardX import SummaryWriter
    # with SummaryWriter(comment='convnext') as W:
    #     W.add_graph(model, (x, ))
    out = model(x)[0]
    print(out.shape)